...
if($_SERVER["REMOTE_ADDR"]==$server_ip || $_SERVER["REMOTE_ADDR"]==$home_ip || !$verify_ip) {
    ...
    if (!empty($api_key) && !empty($api_secret) && !empty($type)) {

        $trading_api_sdk = new TradingApiSdkV4($api_key, $api_secret);

        $response = $trading_api_sdk->doRequest(TradingApiSdkV4::METHOD_SHOW_ACCOUNT_LEDGER, [
            TradingApiSdkV4::SHOW_ACCOUNT_LEDGER_PARAMETER_CURRENCY        => TradingApiSdkV4::CURRENCY_BTC,
            TradingApiSdkV4::SHOW_ACCOUNT_LEDGER_PARAMETER_TYPE            => $type,
        ]);
...
class TradingApiSdkV4
{
    ...
    protected static $method_settings = [
        ...
        self::METHOD_SHOW_ACCOUNT_LEDGER             => [
            ...
            'parameters'   => [
                self::SHOW_ACCOUNT_LEDGER_PARAMETER_TYPE,
            ],
        ],
